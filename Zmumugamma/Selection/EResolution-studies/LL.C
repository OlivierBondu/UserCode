...
// Prepare output file
	...
	TProfile *LL = new TProfile("LL", "LL", 101, 0, 11);
	TProfile *LL_s = new TProfile("LL_s", "LL_s", 101, 0, 11);
	TProfile *LL_mmg = new TProfile("LL_mmg", "LL_mmg", 101, 0, 11);
...
*/
	...
	char x[50];
	double s_alpha;
//	for(double alpha = 0.0 ; alpha <= 10.0 ; alpha += 0.1)
	for(double alpha = 0.0 ; alpha <= 8.0 ; alpha += 0.1)
	{
		sprintf(x,"%2.1f",alpha);
		s_alpha = (alpha - 1) * 100;
		cout << "x = " << x << endl;
		cout << Form("Selected_MC_0%s_eta_k.txt", x) << endl;
...
// Read mc file
		if(alpha != 10.0) RooDataSet *mc = RooDataSet::read(Form("Selected_MC_0%s_eta_k.txt", x), RooArgList(etaMC, mmgMC, kMC, ikMC, sMC));
		if(alpha == 10.0) RooDataSet *mc = RooDataSet::read(Form("Selected_MC_%s_eta_k.txt", x), RooArgList(etaMC, mmgMC, kMC, ikMC, sMC));
...
// Create NLL of MC to data
		...
		*errfile << x << 
		...
		"\t" << sEBnll->getVal() << 
		endl;
		cout << alpha << "\t" << -(mmgEBnll->getVal()) << endl;
		LL_mmg->Fill(alpha, -(mmgEBnll->getVal()));
		cout << alpha << "\t" << -(kEBnll->getVal()) << endl;
		LL->Fill(alpha, -(kEBnll->getVal()));
		cout << alpha << "\t" << -(sEBnll->getVal()) << endl;
		LL_s->Fill(alpha, -(sEBnll->getVal()));
...
// Draw nll
	...
	LL->Draw("E1");
	TF1 *p2kfit = new TF1("p2kfit","pol2", 0.0, 10.0);
	LL->Fit(p2kfit, "Rw");
	double p0, p1, p2;
...
//	th2->Draw();
	...
	LL_s->Draw("E1");
	TF1 *p2sfit = new TF1("p2sfit","pol2", 0.0, 10.0);
	LL_s->Fit(p2sfit, "Rw");
...
//	th2->Draw();
	...
	LL_mmg->Draw("E1");
	TF1 *p2mmgfit = new TF1("p2mmgfit","pol2", 0.0, 10.0);
