...
int main(int argc, char *argv[])
{
	...
	double sigma_ = 0.0;
  if( argc > 1 )
  { // if there is an input value for EResolution
...
// INSERTFILES
  string sample = "SAMPLEPART";
	...
	TFile* OutputRootFile = new TFile(Form("miniTree_%s_%s.root", sample.c_str(), argv[1]), "RECREATE");
	ofstream outfile(Form("SAMPLEPART_%s-alpha.out", argv[1]));
  ofstream errfile(Form("SAMPLEPART_%s-alpha.err", argv[1]));
...
// WARNING for change of resolution
	Float_t Photon_EResFactor;
// WARNING for change of resolution
...
//	unsigned int NbEvents = 1000;
	...
	outfile << "Nb of events : " << NbEvents << endl;
	outfile << "Signal is: " << signal <<endl;
	outfile << "Stew is: " << stew << endl;
	outfile << "ZJet veto is: " << zjet_veto << endl;
...
*/
		...
		{
      errfile << "CUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT for pthat filtering" << endl;
			continue;
		}
   ...
   // Cleaning: removing not commissionned superclusters and spikes
    ...
    vector<double> photonsNoSpikeResolution;
    vector<int> photonIsNotCommissioned;
    photonsNoSpike.clear();
    photonsNoSpikeResolution.clear();
    photonIsNotCommissioned.clear();
    if(verbosity>0) errfile << "\t\tThere is " << NbPhotons << " photons in the photon collection" << endl;
    for(int iphoton=0 ; iphoton<NbPhotons ; iphoton++){
      TRootPhoton *myphoton;
      myphoton = (TRootPhoton*) photons->At(iphoton);

	    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 		  // !!!!!!!!!!!!!!!! WARNING: CHANGING EResolution (photon basis) !!!!!!!!!!!!!!!!!!!
    	// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			Double_t EResolution = g->Gaus(myphoton->Energy(), sigma_);
			Double_t ResFactor = 1.0 + ((double)(EResolution) / (double)(myphoton->Energy()));
    	// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    	// !!!!!!!!!!!!!!!! WARNING: CHANGING EResolution (photon basis) !!!!!!!!!!!!!!!!!!!
    	// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if( (myphoton->superCluster()==0) ){ // 'broken' photon, there should be none
        ...
        if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " rejected because no associated SC" << endl;
        continue;
      }
/*
      if( (1-((myphoton->superCluster()->s4())/(myphoton->superCluster()->eMax()))>0.95) && (myphoton->isEBPho()) ){ // 'spike' photons
        photonIsNotCommissioned.push_back(1);
        errfile << "\t\t\tphoton " << iphoton << " rejected because" << endl;
        continue;
      }
...
*/
      if( myphoton->superCluster()->seedSeverity()==3 ){ // kWeird
        photonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " rejected because kWeird" << endl;
        continue;
      }
      if( myphoton->superCluster()->seedRecoFlag()==2 ){ // kOutOfTime
        photonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " rejected because kOutOfTime" << endl;
        continue;
      }
      if( myphoton->superCluster()->seedSeverity()==4 ){ // kBad
        photonIsNotCommissioned.push_back(1);
        continue;
        if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " rejected because kBad" << endl;
      }

      if( (fabs(myphoton->superCluster()->Eta()))>2.5 ){ // high eta clusters, not commisionned
        photonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " rejected because high eta" << endl;
        continue;
      }
      if( fabs(myphoton->superCluster()->Eta())>1.4442 && fabs(myphoton->superCluster()->Eta())<1.566 ){// eta gap clusters, not commissionned
        photonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " rejected because eta gap" << endl;
        continue;
      }
/*      if ( myphoton->hoe() > 0.05){ // HoE cut is here for trigger issues
      ...
      if(verbosity>0) errfile << "\t\t\tphoton " << iphoton << " accepted" << endl;
      photonsNoSpike.push_back(iphoton);
      photonsNoSpikeResolution.push_back(ResFactor);
    ...
    if(verbosity>0) errfile << "\t\tThere is " << NbMuons << " muons in the muon collection" << endl;
    for(int imuon=0 ; imuon<NbMuons ; imuon++){
      ...
      if(! (mymuon->isGlobalMuon() && mymuon->isTrackerMuon()) ){// The muon must be identified both as Global Muon and Tracker Muon.
        ...
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because not global AND tracker muon" << endl;
        continue;
      }
      if(! (mymuon->numberOfValidTrackerHits()>10) ){// #tracker hits > 10
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because less than 10 tracker hits (" << mymuon->numberOfValidTrackerHits() << ")" << endl;
        continue;
      }
      if(! (mymuon->normalizedChi2()<10) ){// chi2/ndof of the global muon fit < 10
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because chi2/ndof of the global muon fit < 10 (" << mymuon->normalizedChi2() << ")" << endl;
        continue;
      }
/*
      if(! () ){// Number of valid muon-detector hits used in the global fit > 0
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because " << endl;
        continue;// FIXME
      }
*/
/*
      if(! (mymuon->dB()<0.002) ){// Transverse impact parameter of the muon with respect to the beam spot < 2 mm
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because too high transverse impact parameter" << endl;
        continue;
      }
*/
      if(! (fabs(mymuon->Eta())<2.4) ){// |eta_muon|< 2.1
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because high eta (" << mymuon->Eta() << ")" << endl;
        continue;
      }
/*
      if(! (Muon_eventPassHLT_Mu11==1) ){// HLT_Mu11
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because not passing HLT_Mu11" << endl;
        continue;
      }
*/
...
/*
      if(! ((double)(mymuon->isoR03_sumPt() + mymuon->isoR03_emEt() + mymuon->isoR03_hadEt())/(double)(mymuon->Pt())<.15) ){// Relative combined isolation = (sumPt + emEt + hcalEt)/ptmu < 0.15 in a deltaR < 0.3 cone
        muonIsNotCommissioned.push_back(1);
        if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because relative combined isolation bad" << endl;
        continue;
      }
*/
			...
			if(! ((double)(mymuon->isoR03_sumPt()) < 3.0) ){// tracker isolation
				muonIsNotCommissioned.push_back(1);
				if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " rejected because tracker isolation high (" << mymuon->isoR03_sumPt() << " GeV)" << endl;
				continue;
			}

      if(verbosity>0) errfile << "\t\t\tmuon " << imuon << " accepted" << endl;
    ...
    unsigned int NbMuonsIdentified = muonIdentified.size();
		...
		if( stew ){
			if( (event->csa07id() != 62) && (event->csa07id() != 63) ){
				errfile<<"CSA07ID is not 62/63 ( "<< event->csa07id() << ") aborting event " << ievt << endl;
				miniTree->Fill();
				for(int imuon=0 ; imuon<NbMuons ; imuon++){
					TRootMuon *mymuon;
					mymuon = (TRootMuon*) muons->At(imuon);
...
//      if(   MCsignal_in_phase_space ) // ***** WARNING *****  veto currently normal to consider background
    ...
    if( zjet_veto && powheg )
    {
      ...
      {// consistency check
        errfile << "There is " << mcMuMuGammaEvent->nFSR() << " fsr photons in the event" << endl;
...
//        if(   MCsignal_in_phase_space ) // ***** WARNING *****  veto currently normal to consider background
        ...
        outfile << "Failed POWHEG mumugamma consistency check" << endl;
		...
		if(!( NbMuonsIdentified>1 )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level I because "<< NbMuonsIdentified	<< " muons only" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuons ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(imuon);
		...
		if(!( NbMuonsValidEta>1 )) {
			miniTree->Fill();
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level I because of bad muons (eta)" << endl;
		...
		if(!( isThereOppositeCharge )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level I because of bad muons (charge)" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
...
//		subleadingMuon	= (TRootMuon*) SecondMPtMuon;
		...
		errfile << "\t\tINFO: Dimuon invariant mass : Mmumu = " << mumuInvMass << endl;
		mumu.Clear();
		Mmumu = mumuInvMass;


		// CUT 1d: two muons with correct eta with opposite charge AND pT>10 GeV
		if(!( (PtMuon>10.0) && (PtMuon_oppositeCharge>10.0) && ( (fabs(MuonL_Eta)<2.1)||(fabs(MuonS_Eta)<2.1) ) )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level I because of bad muons (ptmu1= " << PtMuon << " GeV; ptmu2= " << PtMuon_oppositeCharge << " GeV)" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( mumuInvMass >= 20.0 )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level I because of m(mumu)= " << mumuInvMass << " GeV" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( NbPhotonsNoSpike>0 )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level II because "<<	NbPhotonsNoSpike	<< " photon" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		vector<double> photonsValidEtaResolution;
		photonsValidEta.clear();
		photonsValidEtaResolution.clear();
		...
		for(int iphoton=0 ; iphoton<NbPhotonsNoSpike ; iphoton++){
			...
			EScale = photonsNoSpikeResolution[iphoton];
...
//			if( (photonValidEtaCandidate->Pt()>0.0) && (fabs(photonValidEtaCandidate->Eta())<2.5) ){ // FIXME
				...
				photonsValidEtaResolution.push_back(photonsNoSpikeResolution[iphoton]);
			}
		}
		NbPhotonsValidEta = photonsValidEta.size();
		if(!( NbPhotonsValidEta>0 )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )" << " CUT at level II because of bad photon" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
			...
			for(int iphoton=0 ; iphoton<NbPhotonsValidEta ; iphoton++){
				...
				EScale = photonsValidEtaResolution[iphoton];
				Pt_allPhotons = EScale*(myphoton->Pt());
				Eta_allPhotons = myphoton->Eta();
				Phi_allPhotons = myphoton->Phi();
				isEBorEE_allPhotons = 1;
		...
		EScale = photonsValidEtaResolution[0];
		Photon_EResFactor = EScale;
		...
		for(int iphoton=0 ; iphoton<NbPhotonsValidEta ; iphoton++){
			...
			if((photonsValidEtaResolution[iphoton]) * (MPtPhotonCandidate->Pt()) > PtPhoton){
...
//		outfile << "MPtPhoton->superCluster()=" << MPtPhoton->superCluster() << endl; // FIXME
//		Photon_nBasicClusters = MPtPhoton->superCluster()->nBasicClusters();
		EScale = Photon_EResFactor;
...
//		mumugamma = (*leadingMuon) + (*subleadingMuon) + (*MPtPhoton);
    ...
    mmg_logk1 = log(mmg_k1);
		...
		if(!( deltaRmin>=0.00 )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"	<< " CUT at level II for Photons (deltar= " << deltaRmin << " )" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( (mumuInvMass>=40.0)&&(mumuInvMass<=80.0) )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"	<< " CUT at level III for Drell-Yan: mumuInvMass= " << mumuInvMass << " GeV" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( cutMuMuGammaWindow )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"	<< " CUT at level V for Z Mass Window: mumugammaInvMass= " << mumugammaInvMass << " GeV" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( farMuon->isoR03_emEt()<=1.0 )) {
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"	<< " CUT at level VI for large emEt: farMuon->isoR03_emEt()= " << farMuon->isoR03_emEt() << " GeV" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( farMuon->Pt()>=30.0 )) {
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"	<< " CUT at level VII for soft far muon: farMuon->Pt()= " << farMuon->Pt() << " GeV" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		if(!( nearMuon->isoR03_hadEt()<1.0 )){
			errfile << "\tCUT: event " << ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"	<< " CUT at level VIII for large hadEt: nearMuon->isoR03_hadEt()= " << nearMuon->isoR03_hadEt() << " GeV" << endl;
			miniTree->Fill();
			for(int imuon=0 ; imuon<NbMuonsValidEta ; imuon++){
				TRootMuon *mymuon;
				mymuon = (TRootMuon*) muons->At(muonsValidEta[imuon]);
		...
		isSelected = 1;
		nSelected++;
		errfile << "OK: Surviving veto event: "<< ievt << " ( " << iRunID << " , " << iLumiID << " , " << iEventID << " )"  << endl;
...
/*
	...
	outfile << endl << "**************************************************************************" << endl;
	outfile << "cut" << "\t\t" << "# events" << "\t\t" << "% rel"	<< "\t\t" << "% abs"<<endl;
	outfile << "nBeforeAllCuts=" << "\t\t" << nBeforeAllCuts << "\t\t" << "-" << "\t\t" << "-" << endl;
	outfile << "nAfterCutPthatFilter=" << "\t\t" << nAfterCutPthatFilter << "\t\t" << (double)nAfterCutPthatFilter/(double)nBeforeAllCuts*100.0 << "\t\t" << (double)nAfterCutPthatFilter/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCutCSA07ID=" << "\t\t" << nAfterCutCSA07ID << "\t\t" << (double)nAfterCutCSA07ID/(double)nBeforeAllCuts*100.0 << "\t\t" << (double)nAfterCutCSA07ID/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCutZJETVETO=" << "\t\t" << nAfterCutZJETVETO << "\t\t" << (double)nAfterCutZJETVETO/(double)nBeforeAllCuts*100.0 << "\t\t" << (double)nAfterCutZJETVETO/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut1a=" << "\t\t" << nAfterCut1a << "\t\t" << (double)nAfterCut1a/(double)nBeforeAllCuts*100.0 << "\t\t" << (double)nAfterCut1a/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut1b=" << "\t\t" << nAfterCut1b << "\t\t" << (double)nAfterCut1b/(double)nAfterCut1a*100.0 << "\t\t" << (double)nAfterCut1b/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut1c=" << "\t\t" << nAfterCut1c << "\t\t" << (double)nAfterCut1c/(double)nAfterCut1b*100.0 << "\t\t" << (double)nAfterCut1c/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut1d=" << "\t\t" << nAfterCut1d << "\t\t" << (double)nAfterCut1d/(double)nAfterCut1c*100.0 << "\t\t" << (double)nAfterCut1d/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut1e=" << "\t\t" << nAfterCut1e << "\t\t" << (double)nAfterCut1e/(double)nAfterCut1d*100.0 << "\t\t" << (double)nAfterCut1e/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut2a=" << "\t\t" << nAfterCut2a << "\t\t" << (double)nAfterCut2a/(double)nAfterCut1e*100.0 << "\t\t" << (double)nAfterCut2a/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut2b=" << "\t\t" << nAfterCut2b << "\t\t" << (double)nAfterCut2b/(double)nAfterCut2a*100.0 << "\t\t" << (double)nAfterCut2b/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut2c=" << "\t\t" << nAfterCut2c << "\t\t" << (double)nAfterCut2c/(double)nAfterCut2b*100.0 << "\t\t" << (double)nAfterCut2c/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut3=" << "\t\t" << nAfterCut3 << "\t\t" << (double)nAfterCut3/(double)nAfterCut2c*100.0 << "\t\t" << (double)nAfterCut3/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut4=" << "\t\t" << nAfterCut4 << "\t\t" << (double)nAfterCut4/(double)nAfterCut3*100.0 << "\t\t" << (double)nAfterCut4/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut5=" << "\t\t" << nAfterCut5 << "\t\t" << (double)nAfterCut5/(double)nAfterCut4*100.0 << "\t\t" << (double)nAfterCut5/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut6=" << "\t\t" << nAfterCut6 << "\t\t" << (double)nAfterCut6/(double)nAfterCut5*100.0 << "\t\t" << (double)nAfterCut6/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut7=" << "\t\t" << nAfterCut7 << "\t\t" << (double)nAfterCut7/(double)nAfterCut6*100.0 << "\t\t" << (double)nAfterCut7/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut8=" << "\t\t" << nAfterCut8 << "\t\t" << (double)nAfterCut8/(double)nAfterCut7*100.0 << "\t\t" << (double)nAfterCut8/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut9=" << "\t\t" << nAfterCut9 << "\t\t" << (double)nAfterCut9/(double)nAfterCut8*100.0 << "\t\t" << (double)nAfterCut9/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nAfterCut10=" << "\t\t" << nAfterCut10 << "\t\t" << (double)nAfterCut10/(double)nAfterCut9*100.0 << "\t\t" << (double)nAfterCut10/(double)nBeforeAllCuts*100.0 << endl;
	outfile << "nSelected=" << "\t\t" << nSelected << "\t\t" << (double)nSelected/(double)nAfterCut10*100.0 << "\t\t" << (double)nSelected/(double)nBeforeAllCuts*100.0 << endl;
