...
// Part 3 : Plots to make
	...
	for (int nbcuts=0 ; nbcuts < NB_CUTS+1 ; nbcuts++) {
		...
		plots_deltar_near[nbcuts] = new TH1D(Form("deltar_near_%i", nbcuts), Form("Deltar NEAR shape after cut %i", nbcuts), 80, 0, 8);
		plots_deltar_far[nbcuts] = new TH1D(Form("deltar_far_%i", nbcuts), Form("Deltar FAR shape after cut %i", nbcuts), 80, 0, 8);
	}  		
	...
	// ISOLATION PLOTS 
	const unsigned int NB_PLOTS=12;
...
//NbEvents=100;
	...
	for(unsigned int ievt=0; ievt<NbEvents; ievt++)	{
		...
		bool CUT0=false, CUT1=false, CUT2=false, CUT3=false, CUT4=false, CUT5=false, CUT6=false, CUT7=false, CUT8=false, CUT9=false, CUT10=false, CUT11=false; 
		...
		// STEP X
		mycorrectedphoton = (TRootPhoton*) correctedPhotons->At(valid_gamma);
		if (tempomuon->isoR03_hadEt() > 1) {
			cerr << "\tCUT: event " << ievt  << " CUT at level X for hadEt muon near" << endl;
			CUT10=true;
		} 
		// PLOTS AT LEVEL 10
		if (CUT10) continue; 
 ...
 // STEP XI
    mycorrectedphoton = (TRootPhoton*) correctedPhotons->At(valid_gamma);
    if (deltar > 1.5) {
      cerr << "\tCUT: event " << ievt  << " CUT at level XI for gamma/mu correlation" << endl;
      CUT11=true;
    }
    // PLOTS AT LEVEL 11
    if (CUT11) continue;
    // BEGIN N-1 PLOTS 
      THESE_MASSE_MUMUGAMMA[11]->Fill(mumugamma->M());
    // END N-1 PLOTS 

    if (!CUT11) {
      Reco_mumugamma_invariant_mass[11]->Fill(mumugamma->M());
      plots_deltar_near[11]->Fill(mydeltar[0]);
      plots_deltar_far[11]->Fill(mydeltar[1]);
      // FILL ISO PLOTS FOR EACH MUONS
      for(int nbofmuons=0 ; nbofmuons < 2 ; nbofmuons++) {
        mymuon = (TRootMuon*) muons->At(valid_muons[nbofmuons]);
        // DR=0.3
        ISO_dr03_emEt[11]->Fill(mymuon->isoR03_emEt());
        ISO_dr03_hadEt[11]->Fill(mymuon->isoR03_hadEt());
        ISO_dr03_hoEt[11]->Fill(mymuon->isoR03_hoEt());
        ISO_dr03_njets[11]->Fill(mymuon->isoR03_nJets());
        ISO_dr03_ntracks[11]->Fill(mymuon->isoR03_nTracks());
        ISO_dr03_sumpt[11]->Fill(mymuon->isoR03_sumPt());
        // DR=0.5
        ISO_dr05_emEt[11]->Fill(mymuon->isoR05_emEt());
        ISO_dr05_hadEt[11]->Fill(mymuon->isoR05_hadEt());
        ISO_dr05_hoEt[11]->Fill(mymuon->isoR05_hoEt());
        ISO_dr05_njets[11]->Fill(mymuon->isoR05_nJets());
        ISO_dr05_ntracks[11]->Fill(mymuon->isoR05_nTracks());
        ISO_dr05_sumpt[11]->Fill(mymuon->isoR03_sumPt());
        // Comparaison DR=0.3 / DR=0.5
        ISO_had_vs_ISO_ntracks_dr03[11]->Fill(mymuon->isoR03_nTracks(), mymuon->isoR03_hadEt());
        ISO_had_vs_ISO_ntracks_dr05[11]->Fill(mymuon->isoR05_nTracks(), mymuon->isoR05_hadEt());
      }
      // FILL the EXTREMS PLOTS 0 = near && 1 =far
      tempomuon = (TRootMuon*) muons->At(deltar_muon[0]);
      tempomuon2 = (TRootMuon*) muons->At(deltar_muon[1]);
      if (tempomuon && tempomuon2) {
        // DR=0.3 && DR=0.5 NEAR
        ISO_extrems_vars_dr03_emEt_near[11]->Fill(tempomuon->isoR03_emEt()); ISO_extrems_vars_dr05_emEt_near[11]->Fill(tempomuon->isoR05_emEt());
        ISO_extrems_vars_dr03_hadEt_near[11]->Fill(tempomuon->isoR03_hadEt());ISO_extrems_vars_dr05_hadEt_near[11]->Fill(tempomuon->isoR05_hadEt());
        ISO_extrems_vars_dr03_hoEt_near[11]->Fill(tempomuon->isoR03_hoEt()); ISO_extrems_vars_dr05_hoEt_near[11]->Fill(tempomuon->isoR05_hoEt());
        ISO_extrems_vars_dr03_njets_near[11]->Fill(tempomuon->isoR03_nJets()); ISO_extrems_vars_dr05_njets_near[11]->Fill(tempomuon->isoR05_nJets());
        ISO_extrems_vars_dr03_ntracks_near[11]->Fill(tempomuon->isoR03_nTracks()); ISO_extrems_vars_dr05_ntracks_near[11]->Fill(tempomuon->isoR05_nTracks());
        ISO_extrems_vars_dr03_sumpt_near[11]->Fill(tempomuon->isoR03_sumPt()); ISO_extrems_vars_dr05_sumpt_near[11]->Fill(tempomuon->isoR05_sumPt());
        // DR=0.3 && DR=0.5 FAR
        ISO_extrems_vars_dr03_emEt_far[11]->Fill(tempomuon2->isoR03_emEt()); ISO_extrems_vars_dr05_emEt_far[11]->Fill(tempomuon2->isoR05_emEt());
        ISO_extrems_vars_dr03_hadEt_far[11]->Fill(tempomuon2->isoR03_hadEt());ISO_extrems_vars_dr05_hadEt_far[11]->Fill(tempomuon2->isoR05_hadEt());
        ISO_extrems_vars_dr03_hoEt_far[11]->Fill(tempomuon2->isoR03_hoEt()); ISO_extrems_vars_dr05_hoEt_far[11]->Fill(tempomuon2->isoR05_hoEt());
        ISO_extrems_vars_dr03_njets_far[11]->Fill(tempomuon2->isoR03_nJets()); ISO_extrems_vars_dr05_njets_far[11]->Fill(tempomuon2->isoR05_nJets());
        ISO_extrems_vars_dr03_ntracks_far[11]->Fill(tempomuon2->isoR03_nTracks()); ISO_extrems_vars_dr05_ntracks_far[11]->Fill(tempomuon2->isoR05_nTracks());
        ISO_extrems_vars_dr03_sumpt_far[11]->Fill(tempomuon2->isoR03_sumPt()); ISO_extrems_vars_dr05_sumpt_far[11]->Fill(tempomuon2->isoR05_sumPt());
        // CORRELATIONS PLOTS for DELTAR = 03 && 05 with muon far
        // DELTAR 0.3
        dr03_hadEt_sumpt[11]->Fill(tempomuon2->isoR03_sumPt() , tempomuon2->isoR03_hadEt());
        dr03_hadEt_ntracks[11]->Fill(tempomuon2->isoR03_nTracks() , tempomuon2->isoR03_hadEt());
        dr03_sumpt_ntracks[11]->Fill(tempomuon2->isoR03_nTracks() , tempomuon2->isoR03_sumPt());
        // DELTAR 0.5
        dr05_hadEt_sumpt[11]->Fill(tempomuon2->isoR05_sumPt() , tempomuon2->isoR05_hadEt());
        dr05_hadEt_ntracks[11]->Fill(tempomuon2->isoR05_nTracks() , tempomuon2->isoR05_hadEt());
        dr05_sumpt_ntracks[11]->Fill(tempomuon2->isoR05_nTracks() , tempomuon2->isoR05_sumPt());
        // OTHERS
        ptmuon_far[11]->Fill(tempomuon2->Pt());
        if(tempomuon->Pt() > tempomuon2->Pt()) {
          ptmuon_high[11]->Fill(tempomuon->Pt());
        } else {
          ptmuon_high[11]->Fill(tempomuon2->Pt());
        }
        ptgamma_valid[11]->Fill(mycorrectedphoton->Pt());
      }
    }
		...
		for(int nbofmuons=0 ; nbofmuons < 2 ; nbofmuons++) {	
			mymuon = (TRootMuon*) muons->At(valid_muons[nbofmuons]);
			// DR=0.3
			ISO_dr03_emEt[12]->Fill(mymuon->isoR03_emEt(), myscale);
			ISO_dr03_hadEt[12]->Fill(mymuon->isoR03_hadEt(), myscale);
			ISO_dr03_hoEt[12]->Fill(mymuon->isoR03_hoEt(), myscale);
			ISO_dr03_njets[12]->Fill(mymuon->isoR03_nJets(), myscale);
			ISO_dr03_ntracks[12]->Fill(mymuon->isoR03_nTracks(), myscale);
			ISO_dr03_sumpt[12]->Fill(mymuon->isoR03_sumPt(), myscale); 
			// DR=0.5
			ISO_dr05_emEt[12]->Fill(mymuon->isoR05_emEt(), myscale);
			ISO_dr05_hadEt[12]->Fill(mymuon->isoR05_hadEt(), myscale);
			ISO_dr05_hoEt[12]->Fill(mymuon->isoR05_hoEt(), myscale);
			ISO_dr05_njets[12]->Fill(mymuon->isoR05_nJets(), myscale);
			ISO_dr05_ntracks[12]->Fill(mymuon->isoR05_nTracks(), myscale);
			ISO_dr05_sumpt[12]->Fill(mymuon->isoR03_sumPt(), myscale); 
			// Comparaison DR=0.3 / DR=0.5
			ISO_had_vs_ISO_ntracks_dr03[12]->Fill(mymuon->isoR03_nTracks(), mymuon->isoR03_hadEt(), myscale);
			ISO_had_vs_ISO_ntracks_dr05[12]->Fill(mymuon->isoR05_nTracks(), mymuon->isoR05_hadEt(), myscale);
		}
		for(int i=0 ; i < 2 ; i++ ) {  
			mymuon = (TRootMuon*) muons->At(deltar_muon[i]); 
