...
totalNBofFilesToBeMigrated=`srmls ${T2machine}${T2manager}${T2filepath}${sampleName} | grep -v " 0 " | awk --field-separator "${sampleName}/" '{print $2}' | wc -l`
totalNBofFilesToBeMigrated=$[${totalNBofFilesToBeMigrated}-3]
counter=0
for file in `srmls -count=50 ${T2machine}${T2manager}${T2filepath}${sampleName} | grep -v " 0 " | awk --field-separator "${sampleName}/" '{print $2}'`
do
#	srmmv ${T2machine}${T2filepath}${sampleName}/${file} file:///${sampleName}/.
	...
	counter=$[${counter}+1]
	echo "(${counter} / ${totalNBofFilesToBeMigrated}) ${file} copied/ERASED"
done
